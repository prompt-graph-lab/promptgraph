...
Candidate Route Creation keeps the source Illustration and Candidates and adds a separator plus derived Illustrations immediately after each targeted source. Its Fresh Preview freezes the complete ordered apply plan, including effective active Candidate records, copied source values, parent Route context, planned labels, and lightweight Candidate file state. Apply rechecks those inputs immediately before history or Project mutation and materializes the stored plan only when it remains fresh. A stale Preview requires a new Fresh Preview and leaves history, Project, and save untouched. Preview examples are display-limited; the apply plan is not.
