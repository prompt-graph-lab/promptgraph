...
def prepare_gallery_variant_promotion_line(
    ...
    """Materialize a detached main-sequence Line without publishing it.

    Callables preserve legacy evaluation: identity is allocated after deepcopy,
    and metadata is evaluated separately for each missing provenance dictionary.
    Existing provenance dictionaries are shallow copies, including empty dicts.
    Validation, history, insertion, graph/session updates and saving belong to
    the caller. Preparation errors propagate without mutating the inputs.
    """
