...
def prepare_gallery_variant_promotion_line(
    ...
    """Prepare an unpublished promotion without changing its source or variant.

    The caller supplies path-aware metadata preparation lazily: each missing
    info dictionary requests its own record, in source-generation then lineage
    order. An ID factory preserves ID allocation after deepcopy in the caller.
    Existing info dictionaries are shallow-copied, retaining nested aliases.
    """
