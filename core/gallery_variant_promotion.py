...
def prepare_gallery_variant_promotion_line(
    ...
    """Materialize a promotion without publishing or mutating its source.

    Suppliers preserve legacy evaluation order: allocate the ID after deepcopy,
    and obtain metadata separately for each missing non-dict provenance record.
    Existing provenance dictionaries are shallow-copied, including empty ones.
    Validation, history, insertion, and persistence belong to the caller.
    """
