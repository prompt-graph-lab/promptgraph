...
def prepare_gallery_variant_promotion_line(
    ...
    """Materialize a promoted Line without publishing or mutating its source.

    The caller validates the asset and owns history, insertion, and persistence.
    Suppliers preserve legacy evaluation order: identity is generated after
    deepcopy, and metadata is evaluated separately for each missing info dict,
    only after parsing and resetting the cloned Line. Supplied info dicts are
    shallow copies, including empty dicts; their nested values remain shared.
    """
