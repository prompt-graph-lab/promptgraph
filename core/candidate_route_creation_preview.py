"""Read-only planning for Candidate Route creation previews."""

from core.candidate_inspection import _candidate_metadata_caption, _candidate_path
...
def build_candidate_route_creation_preview(
    ...
    *,
    ...
    """Describe additions without changing the Project or resolving UI scope."""
    ...
    for line in target_lines:
        candidates = list(active_candidates_for_line(line))
        route_candidates = []
        if not candidates:
        ...
        current_route_label = route_label(project, line, reserved_labels)
        if len(examples) < example_limit:
            examples.append({
                ...
                "route_label": current_route_label,
